...
import com.umc.workbook.aws.s3.AmazonS3Manager;
...
public class ReviewCommandServiceImpl implements ReviewCommandService {
    ...
    @Override
    @Transactional
    public ReviewResponse.CreateReviewResultDTO addReview(Long memberId, Long storeId, ReviewRequest.CreateReviewDTO request, MultipartFile reviewImage) {
        // s3에 이미지 업로드
        String uuid = UUID.randomUUID().toString();
        Uuid savedUuid = uuidRepository.save(Uuid.builder()
                .uuid(uuid).build());
        String imageUrl = s3Manager.uploadFile(s3Manager.generateReviewKeyName(savedUuid), reviewImage);
        log.info("s3에 업로드된 리뷰 이미지 url: {}", imageUrl);

        Member member = memberRepository.findById(memberId).get(); // 멤버 조회
        Store store = storeRepository.findById(storeId).get(); // 가게 조회

        Review review = ReviewConverter.toReview(request, imageUrl);
